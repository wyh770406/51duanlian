...
class BootstrapTopbarList < SimpleNavigation::Renderer::Base
  ...
  def tag_for(item)
    if item.url.nil?
      content_tag('span', dropdown_link_content(item), link_options_for(item).except(:method))
    else
      link_to(dropdown_link_content(item), item.url, link_options_for(item))
    end
  end
  ...
  def link_options_for(item)
    ...
    link_options[:data] ||= {}
    opts[:data] = link_options[:data].merge(dropdown_link_data(item))
    opts.delete(:data) if opts[:data].blank?
    opts
  end
  ...
  def dropdown_link_content(item)
    if include_sub_navigation?(item) && !options[:is_subnavigation]
